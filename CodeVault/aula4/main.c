...
int main()
{
    ...
    if(pthread_create(&t3, NULL, &routine, NULL)) return -1;
    if(pthread_create(&t4, NULL, &routine, NULL)) return -1;

    if(pthread_join(t1, NULL)) return -2;
    if(pthread_join(t2, NULL)) return -2;
    if(pthread_join(t3, NULL)) return -2;
    if(pthread_join(t4, NULL)) return -2;
